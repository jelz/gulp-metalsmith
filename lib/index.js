...
function plugin(type, opts) {
    ...
    function get_metalsmith_obj(contents) {
        ...
        return { contents: new Buffer(contents) };
    }

    function get_metalsmith_obj_with_frontmatter(contents) {
        var parsed = fm(contents);
        return _.extend({}, parsed.attributes, { contents: new Buffer(parsed.body) });
    }
    ...
    function get_json_files(contents) {
        ...
        if (!_.isObject(json_files)) { throw new Error('JSON file does not contain object.'); }
        return json_files;
    }
