...
function plugin(opts) {
  ...
  function processFile(file) {
    ...
    var isUsableJSON = (isJsonDefinition(file) && utf8(contents));

    if (isUsableJSON) {
      var jsonDefinitions = createFilesFromJsonDefinition(contents.toString());

      for (var name in jsonDefinitions) {
        if (jsonDefinitions.hasOwnProperty(name)) {
          jsonDefinitions[name].originalBase = file.base;
          files[name] = jsonDefinitions[name];
        }
      }
    }
      ...
      files[key] = {contents: contents, stat: file.stat};
    }

    if(!isUsableJSON){
      files[key].originalBase = file.base;
    }
  }
  ...
  function pushToStream(transformed) {
    _.forOwn(transformed, function (value, key) {
      s.push(new gutil.File({path: value.originalBase + key, base: value.originalBase, contents: value.contents}));
    });
  }
